  ...
  const handleChangestatus = (event) => {
    ...
    if (searchValue.length >= 2) {
      ...
      if (event.target.value === "Todos") {
        ...
        setFilteredProducts(productCopia.filter(product => product.direccion.includes(estado)))
      }
    }
    else {
      setSelectedOptionstatus(event.target.value);
      ...
      if (event.target.value === "Todos") {
        ...
        const estado = event.target.value;
        setFilteredProducts(products.filter(product => product.direccion.includes(estado)))
      }
    }
  ...
  const handleChange = (event) => {
    ...
    if (searchValue.length >= 2) {
      ...
      if (selectedOptionstatus !== "Todos") {
        productCopia = productCopia.filter(product => product.direccion.includes(selectedOptionstatus))
      }
      if (event.target.value === "Todos") {
        setFilteredProducts(productCopia)
      }
        ...
        setFilteredProducts(productCopia.filter(product => product.numero.includes(elementos)))
      }
    }
    else {
      setSelectedOption(event.target.value);
      ...
      if (selectedOptionstatus === "Todos") {
        ...
        productCopia = products.filter(product => product.direccion.includes(selectedOptionstatus))
      }
      if (event.target.value === "Todos") {
        setFilteredProducts(productCopia)
      }
  ...
  const handleSearch = (e) => {
    ...
    if (selectedOption === "Todos") {
      ...
      if (selectedOptionstatus !== "Todos") {
        ...
        productosCopia = prod.filter(product => product.direccion.includes(selectedOptionstatus))
      }
    }
    else {
      if (selectedOptionstatus === "Todos") {
        productosCopia = products;
        const prod = productosCopia
        productosCopia = prod.filter(product => product.numero.includes(selectedOption))
      }
      else {
        productosCopia = products.filter(product => product.direccion.includes(selectedOptionstatus))
        const prod = productosCopia
        productosCopia = prod.filter(product => product.numero.includes(selectedOption))
      }
    }
          ...
          <div className=" min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border ">
            <div className="p-6  mb-0 bg-white border-b-0 border-b-solid rounded-2xl h-3gl">
              ...
              <div className=' overflow-x-auto sm:flex grid grid-cols-1 pb-3' >
                <h6 className='pr-8'><strong>Doctores</strong></h6>
              ...
              <div className="p-0 overflow-x-auto overflow-y-scroll h-2gl">
                <table className="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
                  <thead className="align-bottom">
                    <tr>
                      <th className="px-6 py-3 font-bold text-left uppercase align-middle bg-transparent border-b border-gray-200 shadow-none text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Doctor</th>
                      <th className="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-gray-200 shadow-none text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Contacto</th>
                      <th className="px-6 py-3 font-bold text-center uppercase align-middle bg-transparent border-b border-gray-200 shadow-none text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Dirección</th>
